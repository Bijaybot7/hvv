"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.VacancySchema = exports.Vacancy = void 0;
const openapi = require("@nestjs/swagger");
const mongoose_1 = require("@nestjs/mongoose");
let Vacancy = class Vacancy {
    static _OPENAPI_METADATA_FACTORY() {
        return { bandName: { required: true, type: () => String }, userId: { required: true, type: () => String }, position: { required: false, type: () => String }, location: { required: false, type: () => String }, experience: { required: false, type: () => String }, description: { required: false, type: () => String } };
    }
};
exports.Vacancy = Vacancy;
__decorate([
    (0, mongoose_1.Prop)({ required: true }),
    __metadata("design:type", String)
], Vacancy.prototype, "bandName", void 0);
__decorate([
    (0, mongoose_1.Prop)({ required: true }),
    __metadata("design:type", String)
], Vacancy.prototype, "userId", void 0);
__decorate([
    (0, mongoose_1.Prop)(),
    __metadata("design:type", String)
], Vacancy.prototype, "position", void 0);
__decorate([
    (0, mongoose_1.Prop)(),
    __metadata("design:type", String)
], Vacancy.prototype, "location", void 0);
__decorate([
    (0, mongoose_1.Prop)(),
    __metadata("design:type", String)
], Vacancy.prototype, "experience", void 0);
__decorate([
    (0, mongoose_1.Prop)(),
    __metadata("design:type", String)
], Vacancy.prototype, "description", void 0);
exports.Vacancy = Vacancy = __decorate([
    (0, mongoose_1.Schema)({ collection: 'vacancy', timestamps: true })
], Vacancy);
exports.VacancySchema = mongoose_1.SchemaFactory.createForClass(Vacancy);
//# sourceMappingURL=vacancy.entity.js.map